...
import com.blue.rchina.bean.Province;
...
import com.blue.rchina.views.WheelView;
...
public class RegisterActivity extends BaseActivity implements View.OnClickListener, GalleryFinal.OnHanlderResultCallback {

    @ViewInject(R.id.activity_register)
    LinearLayout parent;
    ...
    public void initView() {
        ...
        pass_edit.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                pass.setSelected(hasFocus);
            }
        });
        verify_edit.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                verify.setSelected(hasFocus);
            }
        });
        phone_edit.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                phone.setSelected(hasFocus);
            }
        });
        check_edit.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            @Override
            public void onFocusChange(View v, boolean hasFocus) {
                check.setSelected(hasFocus);

                if (!hasFocus){
                    String pass = pass_edit.getText().toString();
                    if (!TextUtils.isEmpty(pass)) {
                        if (!check_edit.getText().toString().equals(pass)) {
                            UIUtils.showToast("请再次确认密码");
                            check_edit.setText("");
                        }else {
                            UIUtils.showToast("密码正确");
                        }
                    }
                }
            }
        ...
        icon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                inflate.findViewById(R.id.camera).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        GalleryFinal.openCamera(100, RegisterActivity.this);
                    }
                });
                ...
                inflate.findViewById(R.id.image).setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        GalleryFinal.openGallerySingle(200, RegisterActivity.this);
                    }
                });
    ...
    public void onClick(View v) {
        ...
        String passStr = pass_edit.getText().toString();
        String verifyStr = verify_edit.getText().toString();
        String checkStr = check_edit.getText().toString();
        String addStr = address_text.getText().toString();
        ...
        switch (v.getId()){
            ...
            case R.id.register:
                ...
                }/*else if (TextUtils.isEmpty(verifyStr)){
                    UIUtils.showToast( "验证码不能为空");
                }*/else {
                    if (!passStr.equals(checkStr)){
                        UIUtils.showToast("两次输入的密码不一致，请确认密码");
                    }else {
                        register(phoneStr, passStr,nameStr,addStr,path);
                    }
                }
                break;
        }
    ...
    private void register(final String phoneStr, final String passStr, String nickName, String region,String path) {
        ...
        UserManager.register(phoneStr, passStr, nickName, region,path, new UserManagerInterface() {
            @Override
    ...
    private void getVerify(String phoneStr) {

        handler.sendEmptyMessage(0);
        x.http().get(new RequestParams(String.format(UrlUtils.N_achieveSMSCode,phoneStr)), new Callback.CommonCallback<String>() {
            @Override
    ...
    private void initPupupwindow() {
        //初始化省市区数据
        initPcaJson();
        provinces = JSON.parseArray(pcaJson, Province.class);
        pList = new ArrayList<>();
        cList = new ArrayList<>();
        province = provinces.get(0).getName();
        provinceItem = provinces.get(0);
        city = provinces.get(0).getCity().get(0).getName();
        cityItem = provinces.get(0).getCity().get(0);
        area = provinces.get(0).getCity().get(0).getArea().get(0);
        for (Province province : provinces)
            pList.add(province.getName());
        for (City city : provinces.get(0).getCity())
            cList.add(city.getName());
        Province p = provinces.get(0);
        final City c = p.getCity().get(0);
        aList = new ArrayList<>();
        aList.addAll(c.getArea());
        ...
        View view = LayoutInflater.from(this).inflate(R.layout.layout_choose_two, null, false);
        final WheelView pWheelView = (WheelView) view.findViewById(R.id.wheel_first);
        final WheelView cWheelView = (WheelView) view.findViewById(R.id.wheel_second);
        final WheelView aWheelView = (WheelView) view.findViewById(R.id.wheel_third);

        pWheelView.setItems(pList);

        pWheelView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            @Override
            public void onSelected(int selectedIndex, String item) {
                province = item;
                provinceItem = provinces.get(selectedIndex - 1);
                cList.clear();
                for (City city : provinces.get(selectedIndex - 1).getCity())
                    cList.add(city.getName());
                cWheelView.setItems(cList);
                cWheelView.setSeletion(0);
//                cityItem = provinceItem.getCity().get(0);
//                city = cityItem.getName();
//                aWheelView.setItems(cityItem.getArea());
//                aWheelView.setSeletion(0);
//                area = cityItem.getArea().get(0);
            }
        });

        cWheelView.setItems(cList);
        cWheelView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            @Override
            public void onSelected(int selectedIndex, String item) {
                if (selectedIndex > provinceItem.getCity().size())
                    return;
                city = item;
                cityItem = provinceItem.getCity().get(selectedIndex - 1);
                aList.clear();
                for (String str : cityItem.getArea())
                    aList.add(str);
                aWheelView.setItems(aList);
                aWheelView.setSeletion(0);
//                area = cityItem.getArea().get(0);
            }
        });
        ...
        view.findViewById(R.id.save).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (province.equals(city))
                    pcaStr = province+"-"+city + "市-" + area;
                else
                    pcaStr = province + "省-" + city + "市-" + area;
                address_text.setText(pcaStr);
                window.dismiss();
            }
        });
...
//        window.showAsDropDown(etAdress);
    ...
    public void onHanlderSuccess(int reqeustCode, List<PhotoInfo> resultList) {
        ...
        icon.setImageBitmap(BitmapUtils.compressBitmap(path));
        if (reqeustCode == 100) {
            /*拍照*/
            //changeIcon(path);
