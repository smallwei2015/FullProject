...
public class SystemBrightUtils {
    ...
    public static int getSystemBright(Activity activity) throws SettingNotFoundException {
        // 获取系统亮度
        int bright = Settings.System.getInt(activity.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS);
        return bright;

    }

    public static void setSystemBright(Activity activity,int systemBrightness){
        // 设置系统亮度
        Settings.System.putInt(activity.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS,systemBrightness);
    }

    public static void setCurBright(Activity activity,float brightValue) {
        WindowManager.LayoutParams lp = activity.getWindow().getAttributes();
        lp.screenBrightness = (brightValue <= 0 ? -1.0f : brightValue / 255f);
        activity.getWindow().setAttributes(lp);
    }
